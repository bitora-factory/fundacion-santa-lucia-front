...
export class DocumentGeneratorService {
  ...
  private async generateWordDocument(templateName: string, resident: ResidentModel, docType: string): Promise<void> {
    ...
    const doc = new Docxtemplater(zip, {
      paragraphLoop: true,
      linebreaks: true,
      nullGetter: () => ''  // Reemplaza variables faltantes con texto vacío
    });
  ...
  private prepareDocumentData(resident: ResidentModel): Record<string, string> {
    ...
    return {
      ...
      TIPO_PAGO: this.enumService.getEnumName('payment', resident.payment) || '',
      PAYMENT_TYPE: this.enumService.getEnumName('payment', resident.payment) || '',
